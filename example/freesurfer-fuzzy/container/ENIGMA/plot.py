...
import display
from parser import LivingParkParser, ENIGMAParser, GeneralParser
...
def threshold_summary_stats(enigma_data, livingpark_data, metric, cohen_d_name):
    ...
    """
    merged_data = pd.merge(enigma_data, livingpark_data, on=["Structure"], how="inner")
...
def save_thresholded_summary_stats(data, metric, filename, cohen_d_name):
    ...
    columns_to_keep = ["Structure", metric, "NAVR_corrected", cohen_d_name]
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["thickness", "area", "subcortical_volume"],
        help="Metric to plot.",
    )
    parser.add_argument(
        "--livingpark_input",
...
def main():
    ...
    for subgroup, data in zip(enigma_data["subgroup"], enigma_data["statistics"]):
        ...
        filename = output_dir / f"{disorder}_{metric}_{subgroup}.png"
        plot_summary_stats(
            data,
        ...
        filename = output_dir / f"{disorder}_{metric}_{subgroup}_thresholded.png"
        plot_summary_stats(
            thresholded_data,
        ...
        filename = output_dir / f"{disorder}_{metric}_{subgroup}_thresholded.csv"
