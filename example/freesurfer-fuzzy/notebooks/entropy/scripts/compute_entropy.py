...
class Segmentations:
    ...
    def __init__(self, iohandler, regexp, dtype, ncpus, verbose, labels=None):
        self._files = None
        self._regexp = regexp
...
def compute_entropy_kernel_kfold(
    ...
    with cp.cuda.Device(gpuid):
        ...
        labels = segmentations.get_labels(segmentations_gpu)
        segmentations_gpu_sub_shape = (
            segmentations_gpu.shape[0] - 1,
        ) + segmentations_gpu.shape[1:]
        segmentations_gpu_sub = cp.zeros(
            segmentations_gpu_sub_shape, dtype=segmentations_gpu.dtype
        )
        for i in chunks:
            pbar = PBar(
                total=labels.size,
            ...
            for label in labels:
                segmentations_gpu_sub = segmentations_gpu[_index != i, :]
                p = cp.mean(segmentations_gpu_sub == label, dtype=np.float32, axis=0)
...
def run_entropy(
    ...
    try:
        labels = np.load(labels, allow_pickle=True).get("arr_0") if labels else None
        segmentations = Segmentations(iohandler, regexp, dtype, ncpus, verbose, labels)
...
def parse_args():
    ...
    parser.add_argument(
        "--command", help="JSON command to run. Takes precedence over regexp and output"
    )
    parser.add_argument(
