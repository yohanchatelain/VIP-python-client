    ...
    for meas in "${MEASUREMENTS[@]}"; do
        ...
        if [ "$meas" == "volume" ]; then
            ...
            for hemi in lh rh; do
                python3 $aparcstats2table --parc aparc.a2009s --hemi "$hemi" --subjects ${SUBJECTS[@]} --meas "$meas" --tablefile "$OUTPUT_DIR/${rep}.${hemi}.aparc.$meas.tsv"
