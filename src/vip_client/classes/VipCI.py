...
class VipCI(VipLauncher):
    ...
    @classmethod
    def init(
        cls, vip_key="VIP_API_KEY", girder_key="GIRDER_API_KEY", verbose=True, **kwargs
    ) -> VipCI:
        """
    ...
    def finish(self, verbose: bool = None) -> None:
        ...
        raise NotImplementedError(
            f"Class {self.__name__} cannot delete the distant data."
        )
    ...
    def _create_dir(cls, path: PurePath, location="girder", **kwargs) -> str:
        ...
        if location == "girder":
            ...
            if not (parentType == "folder"):
                raise ValueError(
                    f"Cannot create folder {path} in '{path.parent}': parent is not a Girder folder"
                )
    ...
    def _init_exec(self) -> str:
        ...
        res_path = self._vip_output_dir / time.strftime(
    ...
    def _save_session(self, session_data: dict, location="girder") -> bool:
        ...
        self._girder_client.addMetadataToFolder(
            folderId=folderId, metadata=session_data
        )
    ...
    def _girder_id_to_path(cls, id: str, type: str) -> PurePosixPath:
        ...
        try:
            return PurePosixPath(
                cls._girder_client.get(f"/resource/{id}/path", {"type": type})
            )
        except girder_client.HttpError as e:
            if e.status == 400:
    ...
    def _parse_input_settings(self, input_settings) -> dict:
        ...
        def get_file_from_item(itemId: str) -> str:
            ...
            files = [f["_id"] for f in self._girder_client.listFile(itemId=itemId)]
