...
class VipLauncher:
    ...
    def session_name(self, name: str) -> None:
        ...
        if re.search(r"[^0-9A-Za-z\-_]+", name):
            raise ValueError(
                "Session name must contain only alphanumeric characters and hyphens '-', '_'"
            )
    ...
    def input_settings(self) -> dict:
        ...
        # Return None if the private attribute is unset
        return (
            self._get_input_settings() if self._is_defined("_input_settings") else None
        )
    ...
    def vip_output_dir(self) -> str:
        ...
        # Return None if the private attribute is unset
        return (
            str(self._vip_output_dir) if self._is_defined("_vip_output_dir") else None
        )
    ...
    def __init__(
        ...
        if (
            any([session_name, output_dir, pipeline_id, input_settings])
            and self.__name__ == "VipLauncher"
        ):
            self._print()
    ...
    def init(cls, api_key="VIP_API_KEY", verbose=True, **kwargs) -> VipLauncher:
        ...
        if not cls._AVAILABLE_PIPELINES:
            cls._printc(
                "(!) Your API key does not allow you to execute pipelines on VIP."
            )
            cls._printc(
                f"    Please join some research group(s) on the Web portal: {cls._VIP_PORTAL}"
            )
    ...
    def launch_pipeline(
        ...
        if not self._check_pipeline_id():
            raise TypeError(
                f"(!) Pipeline identifier could not be checked. Please call {self.__name__}.init()"
            )
        self._print("OK")
        ...
        if not self._is_defined("_vip_output_dir"):
            ...
            self._print("Output directory: ", end="", flush=True)
        ...
        if not self._is_defined("_input_settings"):
            raise TypeError(
                "Please provide input parameters for Session: %s" % self._session_name
            )
            # Check content
    ...
    def finish(self, timeout=300) -> VipLauncher:
        ...
        for wid in self._workflows:
            ...
            if (
                ...
                or not self._exists(
                    PurePosixPath(self._workflows[wid]["outputs"][0]["path"]).parent,
                    location="vip",
                )
            ):
        ...
        if removed_outputs:
            ...
            self._print(
                f"Please run finish() again or check the following path(s) on the VIP portal ({self._VIP_PORTAL}):"
            )
    ...
    def show_pipeline(cls, pipeline_id: str = None) -> None:
        ...
        if pipeline_id:
            pipelines = [
                pipe
                for pipe in cls._AVAILABLE_PIPELINES
        ...
        if not pipelines:  # Case no match
            ...
            # Get header
            pipe_header = (
                f"NAME: {pipeline_def['name']} | VERSION: {pipeline_def['version']}"
            )
            ...
            def display_parameter(param: dict) -> None:
                ...
                cls._printc(
                    f"[{param['type'].upper()}]",
                    cls._clean_html(param["description"]),
                    wrapper=wrapper,
                )
    ...
    def _path_to_delete(self) -> dict:
        ...
        return {self._vip_output_dir: "vip"}
            ...
            for status in report:
                ...
                # Display successful executions
                    ...
                    if len(report[status]) == len(self._workflows):
                        ...
                        self._print(
                            f"{len(report[status])} execution(s) ended with success:"
                        )
                        detail(report[status])
    ...
    def _get_exec_infos(cls, workflow_id: str) -> dict:
        ...
        print(infos)
        if 'output_file' in infos['returnedFiles']:
            outputs=infos["returnedFiles"]['output_file']
        elif 'subjid_output' in infos['returnedFiles']:
            outputs=infos["returnedFiles"]['subjid_output']
        elif infos['returnedFiles'] != {}:
            outputs=infos['returnedFiles'][list(infos['returnedFiles'].keys()).pop()]
        ...
        return {
            ...
            "outputs": []
            if not infos["returnedFiles"]
            else [{"path": value} for value in outputs],
        }
    ...
    def _load(self) -> bool:
        ...
        if missing_props:
            raise TypeError(
                f"The following properties are missing in the backup data:\n\t{', '.join(missing_props)}"
            )
        ...
        if not session_data["session_name"].startswith(self.__name__) and (
            backup_data["session_name"] != session_data["session_name"]
        ):
            raise ValueError(
        ...
        diff = [
            ...
            for prop in session_data
            if (
                (prop not in ["session_name", "workflows"])
        ...
        if diff:
            self._print(
                f"(!) The following properties will be overwritten by session backup:\n\t{', '.join(diff)}\n"
            )
    ...
    def _save_session(self, session_data: dict, location="vip") -> bool:
        ...
        if location != "vip":
            return NotImplementedError(
                f"Location '{location}' is unknown for {self.__name__}"
            )
        # Display
        ...
        self._print()
        if done and is_new:
            self._print(f">> Session was saved in: {vip_file}\n")
        elif done:
            self._print(f">> Session saved\n")
        else:
            self._print(f"(!) Session backup failed\n")
        return done
    ...
    def _check_value(self, attribute: str, new_value) -> None:
        ...
        if getattr(self, attribute) != new_value:
            self._print()
            raise ValueError(
                f"'{attribute.lstrip('_')}' is already set for session: {self.session_name}."
            )
    ...
    def _parse_input_settings(self, input_settings) -> dict:
        ...
        return {key: parse_value(value) for key, value in input_settings.items()}
    ...
    def _check_input_settings(
        self, input_settings: dict = None, location: str = None
    ) -> None:
        """
        ...
        if not self._is_defined("_pipeline_id"):
            raise AttributeError(
                "Input settings could not be checked without a pipeline identifier."
            )
        # Parameter names
    ...
    def _check_input_keys(self, input_settings: dict) -> None:
        ...
        unknown_fields = set(input_settings.keys()) - {  # current parameters
    ...
    def _check_input_values(self, input_settings: dict, location: str) -> None:
        ...
        for param in self._pipeline_def["parameters"]:
            ...
            if not self._is_input_full(value):
                raise ValueError(f"Parameter '{name}' contains an empty value")
    ...
    def _is_input_full(cls, value):
        ...
        """
        if isinstance(value, list) and cls._isinstance(
            value, str
    ...
    def _invalid_chars_for_vip(cls, value) -> list:
        ...
        if isinstance(value, list):
            characters = {
                v for val in value for v in cls._INVALID_CHARS_FOR_VIP.findall(str(val))
            }
        else:
    ...
    def _print(self, *args, min_space=-1, max_space=1, **kwargs) -> None:
        ...
        assert (
            -1 <= min_space <= max_space
        ), f"Wrong min_space ({min_space}) or max_space ({max_space})"
        ...
        def nb_nl_end() -> int:
            ...
            lower, upper = (
                min_space + 1,
                max_space + 1,
            )  # `+1` because the first newline is not a blank line
            # Return the framed value
            return (
                n_end
                if (lower <= n_end <= upper)
                else upper
                if (n_end > upper)
                else lower
            )
