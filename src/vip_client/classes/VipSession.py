...
class VipSession(VipLauncher):
    ...
    def local_output_dir(self) -> str:
        ...
        return (
            str(self._local_output_dir)
            if self._is_defined("_local_output_dir")
            else None
        )
    ...
    def local_output_dir(self, new_dir) -> None:
        ...
        if not isinstance(new_dir, (str, os.PathLike)):
            raise TypeError(
                "Property `local_output_dir` should be a string or os.PathLike object"
            )
    ...
    def local_input_dir(self):
        ...
        # Return None if the private attribute is unset
        return (
            str(self._local_input_dir) if self._is_defined("_local_input_dir") else None
        )
    ...
    def local_input_dir(self, new_dir) -> None:
        ...
        if not isinstance(new_dir, (str, os.PathLike)):
            raise TypeError(
                "`local_input_dir` should be a string or os.PathLike object"
            )
        # Path-ify to account for relative paths
        new_path = Path(new_dir)
        # Check conflicts with private attribute
    ...
    def vip_input_dir(self, new_dir) -> None:
        ...
        if not isinstance(new_dir, (str, os.PathLike)):
            raise TypeError(
                "Property `vip_input_dir` should be a string or os.PathLike object"
            )
        # Path-ify
    ...
    def __init__(
        ...
        super().__init__(
            ...
            verbose=verbose
            and any([output_dir, session_name, pipeline_id, input_settings]),
        )
    ...
    def upload_inputs(self, input_dir=None, update_files=True) -> VipSession:
        ...
        if input_dir:
            ...
            raise TypeError(
                f"Session '{self._session_name}': Please provide an input directory."
            )
        # Check local input directory
        if not self._exists(self._local_input_dir, location="local"):
            raise FileNotFoundError(
                f"Session '{self._session_name}': Input directory does not exist."
            )
    ...
    def download_outputs(
        ...
        if init_timeout != 0:
            self._print(
                "Getting output metadata ",
                ("(timeout: %s) " % str(init_timeout)),
                "... ",
                end="",
                sep="",
                flush=True,
        ...
        for workflow in self._select_workflows(get_status):
            ...
            if not failed:  # All missing files were succesfully downloaded
                ...
                self._print(
                    "%d downloads failed. Waiting for the 2nd try." % len(failed)
                )
        ...
        if not failures:
            ...
            self._print(
                "The following files could not be downloaded from VIP:", end="\n\t"
            )
    ...
    def finish(self, timeout=300) -> VipSession:
        ...
        if self._vip_input_dir != self._vip_dir / "INPUTS" and self._exists(
            self._vip_input_dir, location="vip"
        ):
            self._print(f"(!) The input data are still on VIP:\n\t{self.vip_input_dir}")
            self._print("    They belong to another session.")
            self._print(
                "    Please run finish() from the original session or remove them manually on the VIP portal:"
            )
    ...
    def get_inputs(
        self, session: VipSession, get_pipeline=False, get_settings=False
    ) -> VipSession:
        """
    ...
    def _update_workflows(
        self, get_exec_results: bool = False, timeout: int = None
    ) -> None:
        """
        ...
        for workflow_id in self._workflows:
            ...
            try:
                ...
                self._workflows[workflow_id]["outputs"] = [
                    ...
                    {
                        key: elem[key]
                        for key in ["path", "isDirectory", "size", "exists"]
                        if key in elem
                    }
                    for elem in files
                ]
    ...
    def _select_workflows(self, get_status: list) -> dict:
        ...
        nb_exec = sum(
            [len(report[status]) for status in get_status if status in report]
        )
        nExec = 0
        # Enumerate workflows
        for wid in self._workflows:
            ...
            self._print(
                f"[{nExec}/{nb_exec}] Outputs from: ",
                wid,
                " | Started on: ",
                self._workflows[wid]["start"],
                " | Status: ",
                self._workflows[wid]["status"],
                sep="",
            )
            # Yield current ID
    ...
    def _upload_dir(self, local_path: Path, vip_path: PurePosixPath) -> list:
        ...
        if self._mkdirs(vip_path, location="vip"):
            ...
            files_to_upload = [elem for elem in local_path.iterdir() if elem.is_file()]
            ...
            files_to_upload = [
                elem
                for elem in local_path.iterdir()
        ...
        for local_file in files_to_upload:
            ...
            self._print(
                f"\t[{nFile}/{len(files_to_upload)}] Uploading file: {local_file.name} ({size}) ...",
                end=" ",
            )
        ...
        subdirs = [elem for elem in local_path.iterdir() if elem.is_dir()]
        # Recurse this function over sub-directories
        for subdir in subdirs:
            failures += self._upload_dir(
                local_path=subdir, vip_path=vip_path / subdir.name
            )
    ...
    def _download_parallel(self, files_to_download, unzip):
        ...
        try:
            total_size = "%.1fMB" % sum(
                [file["size"] / (1 << 20) for file in files_to_download.values()]
            )
        except:
            total_size = "unknown"
        # Display
        self._print(
            "%d files to download. Total size: %s."
            % (len(files_to_download), total_size)
        )
    ...
    def _init_download(self, workflow) -> dict:
        ...
        for output in workflow["outputs"]:
            ...
            file = (
                vip_path,
                local_path,
            )  # This key matches the requirements of `vip.download_parallel()`
    ...
    def _save_session(self, session_data: dict, location="local") -> bool:
        ...
        self._print()
        if is_new:
            self._print(f">> Session was saved in: {file}\n")
        else:
        ...
        def parse_value(input):
            ...
            # Case: single input, string or path-like
                ...
                if str(input).startswith(
                    self._SERVER_PATH_PREFIX
                ):  # PurePath.is_relative_to() is unavailable for Python <3.9
                    if self._is_defined("_vip_input_dir"):
